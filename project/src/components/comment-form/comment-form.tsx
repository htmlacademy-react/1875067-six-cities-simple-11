...
function CommentForm(): JSX.Element {
  ...
  const handleFieldChange = (evt: {
    target: { name: string; value: NewReview[keyof NewReview] };
  }) => {
  ...
  return (
    <form className="reviews__form form" action="#" method="post">
      ...
      <textarea className="reviews__textarea form__textarea" id="review" name="review" placeholder="Tell how was your stay, what you like and what can be improved" onChange={handleFieldChange} value={formData.description}></textarea>
      <div className="reviews__button-wrapper">
        <p className="reviews__help">
          To submit review please make sure to set <span className="reviews__star">rating</span> and describe your stay with at least <b className="reviews__text-amount">50 characters</b>.
        </p>
        <button className="reviews__submit form__submit button" type="submit" disabled={(formData.rating === 0) || (formData.description.length < 50)}>Submit</button>
      </div>
    </form>
  );
}
